...
const backgroundMusicControls = fs.readFileSync(new URL('../modules/audio-background-music-controls.js', import.meta.url), 'utf8');
const musicEchoModule = fs.readFileSync(new URL('../modules/audio-music-echo.js', import.meta.url), 'utf8');
...
assert.match(audioInitialization, /this\.bgMusicSmoothGain\.connect\(this\.musicEchoTailGate\)/, 'music echo must receive the shaped background-music signal only');
assert.match(audioInitialization, /this\.musicEchoTailGate\.connect\(this\.musicEchoSend\)/, 'the music tail gate must feed the diffuse reverb send');
...
assert.match(audioInitialization, /this\.pleasureSourceGain\.connect\(this\.pleasureGain\)/, 'pleasure ambience should retain its clean path');
assert.match(audioInitialization, /this\.pleasureSourceGain\.connect\(this\.pleasureEnhancer\)/, 'pleasure ambience should have a dedicated harmonic path');
assert.match(audioInitialization, /this\.pleasureEnhancer\.oversample = '2x'/, 'pleasure harmonic enrichment should use oversampling for cleaner processing');
assert.match(audioInitialization, /this\.pleasureEnhancerGain\.connect\(this\.pleasureBlurDryGain\)/, 'pleasure harmonic enrichment should enter the blur mix');
assert.match(audioInitialization, /this\.pleasureGain\.connect\(this\.pleasureBlurDryGain\)/, 'pleasure ambience should use its own blur mix');
...
const mantraBlock = mantraPlayback;
assert.match(app, /playMantraTrack\(key\) \{\s*return audioMantraPlayback\.play\(this, key/);
assert.match(mantraBlock, /owner\.stopMantraTrack\(\{ restoreMusic: false, invalidate: false \}\)/);
assert.match(mantraBlock, /owner\.muteBackgroundMusicForMantra\(musicFadeSeconds\)/);
assert.ok(
    mantraBlock.indexOf('owner.mantraBuffer[key] = await owner.ctx.decodeAudioData(await response.arrayBuffer());') <
    mantraBlock.indexOf('owner.muteBackgroundMusicForMantra(musicFadeSeconds);'),
...
assert.match(mantraBlock, /owner\.restoreBackgroundMusicAfterMantra\(\)/, 'failed mantra loading must release the music mute');
assert.match(mantraBlock, /owner\.masterGain\.gain\.linearRampToValueAtTime\(state\.volDrone \* 0\.15/, 'drone ducking should remain separate from music muting');
assert.match(mantraBlock, /owner\.mantraLoop\.stop\(fadeSeconds\)/, 'loop owns one duration-aware exit fade');
assert.match(mantraBlock, /owner\.restoreBackgroundMusicAfterMantra\(fadeSeconds\)/, 'music returns during the mantra exit fade');
...
const backgroundFade = backgroundMusicControls;
assert.match(app, /fadeInBackgroundMusic\(duration = 4, isDucked = false\) \{\s*return audioBackgroundMusicControls\.fadeIn\(this, duration, isDucked, state\)/);
assert.match(backgroundFade, /if \(now >= owner\.bgMusicEntryEndsAt\) owner\.bgMusicLoop\.setGain\(1\.0\);/, 'ordinary gain adjustments must not cancel the protected slow-entry envelope');
...
assert.match(mantraBlock, /if \(invalidate\) owner\.mantraRequestId \+= 1/, 'stopping should invalidate pending mantra startup');
assert.match(mantraBlock, /if \(requestId !== owner\.mantraRequestId \|\| state\.noMantraMode\) return;/, 'a cancelled handoff must not start the mantra');
assert.match(backgroundFade, /param\.cancelAndHoldAtTime\(now\)/, 'the music fade should preserve the live gain value without a jump');
assert.match(backgroundFade, /const fadeDuration = Math\.max\(0, duration\)[\s\S]*?setBusGain\(owner, 0, fadeDuration\)/, 'music mute should fade the complete music bus to silence');
