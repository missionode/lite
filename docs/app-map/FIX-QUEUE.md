...
CP-MOD-012 through CP-MOD-014 are integrated on `modularize`: Body Scan, Guided Noting and Dharana each have a directly tested lifecycle module. CP-MOD-015 now moves Box Breathing's tutorial, four-cycle practice, pause accounting and music handoff to `modules/box-breathing-practice.js`. These modules remain eager and precached. The Box Breathing 100 ms pause-check loop is deliberately unchanged, so none of these ownership extractions claims a performance gain. Device playback/thermal evidence remains open. Lazy loading stays gated by extraction parity and repeated cold/warm/offline measurements.
