...
- Active language work: branch `add-hindi-language` is based on production `2a7f27b`. Hindi is now a complete `hi-IN` UI/narration language with browser TTS only; no Hindi Piper model is registered, pending separate licence approval as a later enhancement. Hindi meditation sessions do not expose or schedule the Earn handoff. `locales/hi.json` and all Hindi narration fields are registered atomically, avoiding the earlier partial `hi.json` request. Static Hindi locale/narration parity and handoff-suppression checks pass; native Hindi and target-device listening remain release gates. See `TEMP-MULTILINGUAL-ARCHITECTURE.md` → “Hindi (`hi` / `hi-IN`)”.
