...
// for a relative WASM request.
const ONNX_BASE = new URL("../ort/", import.meta.url).href;
...
async function fetchBlob(url, callback) {
  ...
  if (!res.ok) {
    throw new Error(`Piper asset request failed (${res.status}) for ${url}`);
  }
...
const _TtsSession = class _TtsSession {
  ...
  async init() {
    ...
    __privateSet(this, _ort, await import("../ort/ort.min.js"));
    __privateGet(this, _ort).env.allowLocalModels = false;
    // Keep mobile CPU use bounded; the meditation worker should not occupy
    // every core while background music and UI remain active.
    __privateGet(this, _ort).env.wasm.numThreads = globalThis.crossOriginIsolated === true
  ...
  async predict(text, settings = {}) {
    ...
    const lengthScaleMax = Number.isFinite(requestedLengthScaleMax)
    ...
    // Single-speaker voices (including the Malayalam medium voices) may omit
    // speaker_id_map or set it to null. Only multi-speaker models need sid.
    const speakerIdMap = __privateGet(this, _modelConfig).speaker_id_map || {};
