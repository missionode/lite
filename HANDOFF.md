...
- Fullscreen journey chrome: the bottom controls and mirrored top countdown rings fade out by default, reveal on desktop hover or keyboard focus, and recede after pointer departure. This applies only while the persistent app container is fullscreen; ordinary journeys retain their existing control visibility. Validation: `static/unit` PASS — JavaScript/service-worker syntax, journey-video-prelude, Hindi, background music, spatial audio, and diff check.
