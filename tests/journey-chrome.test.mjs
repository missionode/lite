...
assert.match(html,/modules\/journey-chrome\.js\?v=1\.0[\s\S]*?app\.js\?v=4\.04/,'Shared journey chrome must load before the app constructs its controller.');
assert.match(serviceWorker,/chakra-v5\.300[\s\S]*?modules\/journey-chrome\.js\?v=1\.0/,'The shared chrome module must remain available offline.');
