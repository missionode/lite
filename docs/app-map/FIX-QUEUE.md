...
CP-MOD-012 through CP-MOD-015 are integrated on `modularize`: Body Scan, Guided Noting, Dharana and Box Breathing each have directly tested lifecycle modules. CP-MOD-016 now extracts Visualization's blackout, audio/narration ordering, optional-ambience fallback, wake prompt, timed practice, return fades and cleanup into `modules/visualization-practice.js`. These modules remain eager and precached; existing practice clocks and fade intervals are preserved, so none of these ownership extractions claims a performance gain. Device playback/thermal evidence remains open. Lazy loading stays gated by extraction parity and repeated cold/warm/offline measurements.
