...
const toneSource=fs.readFileSync('modules/audio-tone-playback.js','utf8');
const droneSource=fs.readFileSync('modules/audio-drone-start.js','utf8');
const state={noFrequencyMode:false,eyesCloseMode:false,volDrone:.05};
const toneContext=vm.createContext({Math,Number,Object,window:{}});vm.runInContext(toneSource,toneContext);
const audioTonePlayback=toneContext.window.ChakraAudioTonePlayback;
vm.runInContext(droneSource,toneContext);
const audioDroneStart=toneContext.window.ChakraAudioDroneStart;
const methods=vm.runInNewContext('(class {'+app.slice(app.indexOf('    startDrone(baseFreq'),app.indexOf('    stopGuidedTransitionTone('))+'}).prototype',{state,audioTonePlayback,audioDroneStart});
