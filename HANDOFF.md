...
- New static regression coverage checks supplied-media presence, full-viewport layout, paused ready state, explicit user-start contract, video/audio fade contract, Web Audio routing, failure handoff, restart ordering, localization, and non-precache behavior. Validation: `static/unit` PASS — JavaScript/service-worker syntax, journey-video-prelude, background music, spatial audio, narration ticker, Hindi, Russian, No Frequency, focused practices, and diff check. No Playwright, screenshots, or device listening are run by owner direction.
