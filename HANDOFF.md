...
- In progress: `audio/background_music.mp3` has a local, uncommitted replacement. GitHub Pages was verified to still serve the previous production asset (30 August, 744,610 bytes, ETag `"6a948cc3-b5ca2"`), because the replacement had correctly been excluded from earlier feature commits. This checkpoint publishes that supplied replacement and fetches it through a release-versioned URL (`?v=20260831.1`) with a matching service-worker cache entry, preventing an installed PWA from reviving the old fixed-path cache.
- Echo repair: voice and background-music space now use separate, filtered feedback-delay buses rather than randomized convolution. Both feedback gains are bounded below unity, automate with 250 ms transitions, retain dry signal clarity, and pass through the existing music/mantra gates. The music send now originates after the music EQ/smoothing path, so echo tone matches the dry bed.
