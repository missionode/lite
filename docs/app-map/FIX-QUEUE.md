...
CP-MOD-012/013 are integrated on `modularize`: Body Scan lives in `modules/body-scan-practice.js`; Guided Noting's timed reminders and black-scene cleanup live in `modules/guided-noting-practice.js`. CP-MOD-014 now moves Dharana's anchor/veil lifecycle, one-second shrink updates, narrated release and cleanup into `modules/dharana-practice.js`. All remain eager and precached. These are ownership/testability improvements, not measured performance gains. Device playback/thermal evidence remains open. Lazy loading remains gated by extraction parity and repeated cold/warm/offline measurements.
