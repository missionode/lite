...
- Delivered pending checkpoint: English Piper Lessac has a model-specific meditation character without affecting Malayalam Piper or browser voices. The runtime now permits a registry-bounded `lengthScale` of up to `1.5` (instead of silently capping this voice at `1.35`), allowing the feminine journey profile to reach its intended slower `~0.67` effective pace. Lessac alone also uses restrained inference smoothing (`noiseScale: 0.60`, `noiseW: 0.70`) and a warmer/softer journey profile (warmth 88, clarity 22, pace 0.90). The runtime clamps all supplied values and does not expose unbounded voice controls.
- Validation pending checkpoint: narration-ticker, background-music, and spatial-audio static contracts; app/runtime/service-worker syntax; Piper registry JSON; and `git diff --check` PASS. No Playwright/screenshots by owner direction; target-device listening is still the quality gate.
