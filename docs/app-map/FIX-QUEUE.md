...
CP-MOD-012 is integrated on `origin/modularize`: `modules/body-scan-practice.js` owns Body Scan narration order, duration-based intervals, active-session guards and black-scene cleanup. CP-MOD-013 now extracts Guided Noting's timed reminders and black-scene cleanup into `modules/guided-noting-practice.js`; focused direct contracts and shell-delivery checks cover both seams. The new module is still eager and precached; this is an ownership/testability improvement, not a measured performance gain. Device playback/thermal evidence remains open. Lazy loading remains gated by parity and repeated cold/warm/offline measurements.
