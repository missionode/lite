...
CP-MOD-012 through CP-MOD-017 are integrated on `modularize`: Body Scan, Guided Noting, Dharana, Box Breathing, Visualization and Ho’oponopono each have directly tested lifecycle modules. CP-MOD-018 extracts Undo & Unlearn's black-scene setup, localized phases, active-session guards and cleanup into `modules/undo-unlearn-practice.js`; its `finally` path also hides the scene and releases body mode if the fade wait rejects. All modules remain eager and precached; existing practice timing is preserved, so no performance gain is claimed. Device playback/thermal evidence remains open. Lazy loading stays gated by extraction parity and repeated cold/warm/offline measurements.
