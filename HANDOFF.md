...
- In progress follow-up: owner correctly identified that audible feedback-delay repetitions are not the desired character. Voice and Music Space are being changed to deterministic, filtered diffuse reverb tails with no feedback loop: the effect should blur and fade naturally rather than repeat words or beats. The app and shell cache versions are bumped for installed PWA delivery.
