...
CP-MOD-012 through CP-MOD-018 are integrated on `modularize`: all seven preparation practices and integration practices now have directly tested lifecycle owners; Undo & Unlearn guarantees cleanup when its fade wait rejects. CP-MOD-019 extracts shared screen switching into `modules/screen-navigation.js`, preserving static-decorations guards, the decoration event, visibility and scroll-reset behavior. These modules remain eager and precached; no performance gain is claimed. Browser/device evidence limitations remain in the checkpoint review. Lazy loading stays gated by extraction parity and repeated cold/warm/offline measurements.
