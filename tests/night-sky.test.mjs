...
assert.match(app, /celestialSunEquatorial\(days, date\)[\s\S]*?observer\.approximate \|\| sunAltitude <= -6[\s\S]*?name: 'Sun', kind: 'sun'[\s\S]*?name: 'Earth', kind: 'earth'[\s\S]*?this\.celestialDaylight = false/,
    'Granted-location skies should replace Moon, planets and named stars with a calculated Sun plus the low-horizon Earth theme marker before civil twilight without turning Equator fallback coordinates into false daylight');
assert.match(app, /if \(this\.celestialDaylight\)[\s\S]*?createLinearGradient[\s\S]*?rgba\(35, 48, 124, 0\.34\)[\s\S]*?body\.kind === 'earth'[\s\S]*?255, 250, 230/,
    'Daylight should retain a cached indigo cosmic wash, a gentle warm Sun and an Earth theme marker rather than switching to an ordinary daytime background');
