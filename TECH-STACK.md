...
- Consent presentation is explicitly staged: review first, then selfie teleprompter recording. The prompt DOM is moved into the recording overlay rather than duplicated, preserving one source of truth for text and scroll position. The user can adjust 8–24 px/sec before recording; 10 px/sec is the default Slow setting.
...
- The generated video contract is plan-first and consent-last. MediaRecorder starts with five-second silent session-plan canvas pages, then holds on the transition until the manual consent action and five-second countdown complete. Microphone gain and the consent timer begin only afterward; teleprompter motion waits another ten seconds so the client can read the initially visible lines.
...
- Consent capture separates silent plan generation from spoken recording with a manual second Record action followed by a visible five-second countdown. The spoken recording then holds the prompt stationary for ten seconds before auto-scroll begins at the default Slow rate of 10 px/sec. Exported plan, waiting, and spoken frames share an ivory-and-gold luxury document system, and spoken evidence exposes both elapsed and live local time.
...
- A consent-recorded consultation locks the Meditation Room Core Practice Duration and exposes the active client's individual chakra timings plus their exact chakra-practice total. A non-empty consultation chakra selection defines the active subset; an empty consultation selection falls back to the Meditation Room selection. The displayed session estimate sums the resolved individual timings. `End Consultation and Reset Session` clears client plan/consent data and in-memory recording, restores standard session defaults, and re-enables the five-minute Core timer. Individual durations are generated independently and capped at seven minutes.
