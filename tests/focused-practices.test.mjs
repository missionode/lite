...
for (const id of ['box-breathing-experience-toggle', 'hooponopono-experience-toggle']) {
...
assert.match(app, /getFocusedExperience\(\) \{[\s\S]*?box-breathing-experience-toggle[\s\S]*?hooponopono-experience-toggle/, 'one focused practice should be selected at a time');
assert.match(app, /if \(focusedExperience\) \{[\s\S]*?this\.audio\.fadeInBackgroundMusic\(4\);[\s\S]*?focusedExperience === 'box'[\s\S]*?this\.runBoxBreathing\(\)[\s\S]*?this\.runHooponopono\(\)[\s\S]*?this\.finish\(\);/, 'focused practices should finish directly without a chakra sequence');
...
for (const locale of [en, ml]) {
    for (const key of ['boxBreathingExperience', 'hooponoponoExperience', 'beginBoxBreathing', 'beginHooponopono', 'roadmapBoxBreathing']) {
