...
CP-MOD-012 through CP-MOD-021 are integrated on `modularize`: all journey practices have directly tested lifecycle owners, shared screen switching preserves the Lobby/Settings dynamic-sky exception, Lobby session-estimate calculations are isolated, and Mood & Relaxation ambience control rendering is owned by a directly tested view module. CP-MOD-022 extracts drone-duration Settings radio/note synchronization while app timing, summary and persistence logic remain. Modules remain eager and precached; no performance gain is claimed. Browser/device evidence limitations remain in checkpoint review. Lazy loading stays gated by extraction parity and repeated cold/warm/offline measurements.
