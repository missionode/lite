...
async function capture(page, cdp, phase) {
  const pageMetrics = await page.evaluate(() => {
    ...
    const sky = scripts.filter(entry => /\/(?:astronomy\.browser\.min|sky-stars|sky-astronomy|night-sky|celestial-presence)\.js$/.test(entry.name));
