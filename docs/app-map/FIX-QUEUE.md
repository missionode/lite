...
CP-MOD-012 through CP-MOD-019 are integrated on `modularize`: all journey practices have directly tested lifecycle owners, and shared screen switching preserves the Lobby/Settings dynamic-sky exception, decoration event and scroll resets. CP-MOD-020 extracts Lobby session-estimate priority/formulas into `modules/session-estimate.js`; it preserves labels and values while the app retains rendering and roadmap refresh. Modules remain eager and precached; no performance gain is claimed. Browser/device evidence limitations remain in checkpoint review. Lazy loading stays gated by extraction parity and repeated cold/warm/offline measurements.
