...
- Active feature branch: `add-language-feature`, created from the frozen production checkpoint `a9fcb1e`. The approved Russian implementation is local-only and must not be pushed or merged until the owner requests it and the completed diff is compared against this baseline.
