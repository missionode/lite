...
CP-MOD-012 through CP-MOD-020 are integrated on `modularize`: all journey practices have directly tested lifecycle owners, shared screen switching preserves the Lobby/Settings dynamic-sky exception, and Lobby session-estimate priority/formulas are isolated in `modules/session-estimate.js`. CP-MOD-021 extracts Mood & Relaxation ambience settings DOM synchronization into a frozen module while the app retains live state, localization and audio services. These modules remain eager and precached; no performance gain is claimed. Browser/device evidence limitations remain in checkpoint review. Lazy loading stays gated by extraction parity and repeated cold/warm/offline measurements.
