...
CP-MOD-012 through CP-MOD-022 are integrated on `modularize`: all journey practices have directly tested lifecycle owners, shared screen switching preserves the Lobby/Settings dynamic-sky exception, Lobby session-estimate calculations are isolated, and Mood & Relaxation plus drone-duration settings presentation have directly tested view owners. CP-MOD-023 extracts the central Lobby experience-visibility controller while app services retain state, selection and adjacent calculations. Modules remain eager and precached; no performance gain is claimed. Browser/device evidence limitations remain in checkpoint review. Lazy loading stays gated by extraction parity and repeated cold/warm/offline measurements.
