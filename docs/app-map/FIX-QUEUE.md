...
CP-MOD-054 is integrated on `modularize`: binaural/drone stop cleanup is a separately tested audio owner. CP-MOD-055 is in progress on its isolated checkpoint branch, extracting recorded mantra start/stop. Continue music playback ownership only where its shared-loop retirement/mute/restore state admits a clean seam. Keep all audio modules eager and precached until parity is complete; the final lazy-loading phase still requires repeatable cold/warm/offline browser evidence and explicit opt-in browser verification.
