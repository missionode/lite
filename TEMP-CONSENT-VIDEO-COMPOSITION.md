...
- Plan video data comes from an explicit approved-plan allowlist rather than copying the complete Guide Review DOM. The current verification pages intentionally include client contact, email, citizenship, emergency contact, and full meditation-language name; medication details, private reflection notes, and guide notes remain excluded.
...
- Review Recording can hand the generated WebM, localized title, and covering text to the standard device share sheet. Destination and recipient selection remain with the user; there is no backend delivery or upload.
- The spoken declaration records the date but no longer asks the client to read latitude/longitude. Coordinates remain internal evidence and may still appear in the generated footer.
