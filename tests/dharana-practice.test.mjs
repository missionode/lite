...
assert.match(app, /const dharanaPractice = window\.ChakraDharanaPractice/);
...
assert.match(html, /modules\/dharana-practice\.js\?v=1\.0[\s\S]*?app\.js\?v=3\.80/, 'the module should load before app.js with a refreshed app version');
assert.match(serviceWorker, /chakra-v5\.275[\s\S]*?modules\/dharana-practice\.js\?v=1\.0/, 'the offline shell should precache the same module version');
