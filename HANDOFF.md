...
- EXTENDED locally, app 2.94 / shell 5.180, based on 557a02a. For a normal standard journey with Returning Journey unchecked, Begin now proceeds directly to the illustrated, localized orientation before Arriving. The prior title, lead copy, setup controls and blank return screen are removed. A static standing body-map has each chakra name and body location localized on screen while one expanded narration explains it, then the app explicitly switches to Arriving. No SVG or animation loop remains.
