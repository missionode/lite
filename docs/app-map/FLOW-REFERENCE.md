...
| Clear center pocket? | Choose one Earth size for the current viewport width, then measure visible mantra, labels and controls only on cached-layer rebuilds or explicit layout events. Scrolling may move or temporarily omit Earth for clearance, but never resizes it. Reserve the final position of sliding controls and keep Earth on the center axis. |
...
| No clear pocket | Omit the fixed-size marker if its glow/name would cover text or controls. Re-evaluate on scroll, screen/label/control changes or resize, not an idle polling timer. |
