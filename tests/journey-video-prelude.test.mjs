...
assert.match(prelude, /getVideoBufferTargetSeconds\(\)[\s\S]*?effectiveType[\s\S]*?downlink/, 'the prelude should adapt its initial buffer target to connection conditions');
assert.match(prelude, /async bufferVideoToSafePoint\(\)[\s\S]*?readyState < HTMLMediaElement\.HAVE_FUTURE_DATA[\s\S]*?getBufferedAheadSeconds\(\)[\s\S]*?JOURNEY_VIDEO_PRELUDE_BUFFER_STABILITY_MS/, 'the prelude should wait for a measured stable initial buffer');
assert.match(prelude, /Slow first-use delivery is not a media failure[\s\S]*?setTimeout\(\(\) => finish\(true\), 90000\)/, 'buffer timeout must not silently skip the video prelude');
assert.match(prelude, /getBufferedAheadSeconds\(\)[\s\S]*?JOURNEY_VIDEO_PRELUDE_REBUFFER_SECONDS|JOURNEY_VIDEO_PRELUDE_REBUFFER_SECONDS[\s\S]*?getBufferedAheadSeconds\(\)/, 'the prelude should define a playback rebuffer safety threshold');
...
assert.match(prelude, /isWaitingForBuffer[\s\S]*?JOURNEY_VIDEO_PRELUDE_RESUME_BUFFER_SECONDS/, 'the prelude should resume only after a meaningful recovery buffer');
assert.match(prelude, /remaining <= JOURNEY_VIDEO_PRELUDE_REBUFFER_SECONDS\) return/, 'short clips should not enter impossible end-of-video recovery');
assert.match(prelude, /addEventListener\('waiting', onWaiting\)[\s\S]*?addEventListener\('stalled', onWaiting\)/, 'native media stalls should enter the recovery path');
assert.match(prelude, /this\.playButton\.hidden = true[\s\S]*?bufferVideoToSafePoint\(\)\.then[\s\S]*?this\.playButton\.hidden = false/, 'the Begin control should appear only after the safe buffer is ready');
assert.match(prelude, /bufferCountdownTimer[\s\S]*?getVideoBufferTargetSeconds\(\)[\s\S]*?getBufferedAheadSeconds\(\)/, 'the prelude should show a live buffer countdown while loading');
assert.match(css, /\.journey-video-prelude\.is-playing,[\s\S]*?cursor: none/, 'the pointer should be hidden during cinematic playback');
assert.match(prelude, /is-meditator[\s\S]*?setTimeout\(\(\) => \{[\s\S]*?is-video/, 'the meditator image should dissolve into the video after a short hold');
assert.match(prelude, /this\.media\.load\(\)/, 'the supplied video should be explicitly prepared for native buffering after a user request');
assert.doesNotMatch(prelude.slice(prelude.indexOf('constructor('), prelude.indexOf('getVideoBufferTargetSeconds()')), /this\.media\?\.load\(\)/, 'constructing the prelude must not start loading the optional video');
assert.match(prelude, /ensureVideoSource\(\)[\s\S]*?this\.media\.src = source[\s\S]*?async bufferVideoToSafePoint\(\)\s*\{\s*if \(!this\.ensureVideoSource\(\)\)/, 'selected playback attaches the deferred media URL before buffering');
...
assert.match(prelude, /class JourneyVideoPrelude[\s\S]*?async previewAudio\(\)[\s\S]*?this\.audio\.fadeJourneyVideoPrelude\(state\.volVideo, 0\.25\)[\s\S]*?const onPlay = \(\) => \{[\s\S]*?const playback = this\.media\.play\(\)[\s\S]*?fadeJourneyVideoPrelude\(state\.volVideo, JOURNEY_VIDEO_PRELUDE_FADE_IN_SECONDS\)/, 'preview and actual video playback should use the independent Video Volume');
...
assert.match(app, /function validateLobbyStartBeforePrelude\(\)[\s\S]*?journeyRouting\.resolveLaunchRoute[\s\S]*?journeyRouting\.validateLobbyStart[\s\S]*?Please select at least one chakra[\s\S]*?startMeditationBtn\.addEventListener\('click', async \(\) => \{\s*if \(!validateLobbyStartBeforePrelude\(\)\) return;[\s\S]*?state\.journeyVideoPreludeEnabled/,
...
assert.match(prelude, /this\.media\.pause\(\);[\s\S]*?this\.audio\.fadeJourneyVideoPrelude\(0, 0\);/, 'the ready screen should leave the video paused and silent before Play');
...
const journeyVideoPrelude = prelude;
